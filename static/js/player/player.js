...
export class Player extends AcGameObject {
    constructor(root, info) {
        ...
        this.gravity = 50;
        this.direction = 1;
        this.status = 3; // 0 不动， 1表示向前走， 2表示向后， 3表示跳跃， 4表示攻击， 5被打， 6死亡
    ...
    control() {
        ...
        if (this.id === 0) {
            ...
            space = this.pressed_keys.has('j');
        } else {
            w = this.pressed_keys.has('ArrowUp');
            a = this.pressed_keys.has('ArrowLeft');
            d = this.pressed_keys.has('ArrowRight');
            space = this.pressed_keys.has('1');
        }
    ...
    update_direction() {
        ...
        let ps = this.root.players;
        if (ps[0] && ps[1]) {
            let me = this, you = ps[1 - this.id];
            if (me.x < you.x) {
                me.direction = 1;
            } else {
                me.direction = -1;
    ...
    render() {
        ...
        // } else {
        //     this.ctx.fillStyle = 'red';
        //     this.ctx.fillRect(this.x - 120 - 100 + this.width, this.y + 40, 100, 20);
        // }
